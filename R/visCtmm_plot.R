...
visOcc_map <- function(traj_model, img_filename, info){
  image <- terra::rast(paste0(img_filename, '.tif'))
  ...
  terra::plotRGB(image, ext = terra::ext(akde_rast_reproject), legend.title='occurence distribution', mar=c(3.1, 3.1, 2.1, 7.1), stretch='lin', axes=TRUE)
